export const config = {
    teamNames: {
        fullName: 'Stanningley Park Bowling Club',
        shortName: 'Stanningley',
    },
    clubsForPlayersStats: ['stanningley', 'littlemoor', 'pudsey'],
    socialLinks: {
        instagramUrl: 'https://www.instagram.com/StanningleyParkBowls',
        facebookUrl: 'https://www.facebook.com/StanningleyBowlsClub',
        emailUrl: 'mailto:stanningleybowlsclub@gmail.com',
        teamPhotosUrl:
            'https://photos.google.com/share/AF1QipMmkXau_XJaD1459y7Sdsk7pSoUeAr04SiVP_z07i1u0NzLiVQLBioFHwGF9jHV_A?key=N3VLT0l0VWZEVlNrVm5kQUF3MHl1d2lJNEFDRVNR',
        historicStatsUrl:
            'https://photos.google.com/share/AF1QipOqs9EK2LBhL3uZjas8l1ccFkrkdsY8KpOlajx60sBsUaM_-S4LCr-qLpEj9aRX3Q?key=N1VwMnRwdWlKQU5pcW9lVXk1b09IX3Y1OGxQWlFR',
        googleMapsUrl: 'https://goo.gl/maps/RQ7wemtuHQhXPgmF9',
        googleMapsIframeUrl:
            'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d739.5172794483534!2d-1.6554462674593056!3d53.81026923756449!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x48795fb1c49a2601%3A0xe4eb67d3c4eeb88f!2sStanningley%20Park%20Crown%20Green%20Bowling%20Club!5e0!3m2!1sen!2suk!4v1657105300205!5m2!1sen!2suk',
    },
    allTeamsInLeaguesSince2013: [
        // Leagues for club
        'leeds monday combined',
        'airewharfe monday',
        'airewharfe monday (a)',
        'airewharfe monday (b)',
        'leeds tuesday vets',
        'leeds tuesday',
        'leeds half holiday',
        'bradford half holiday',
        'bradford half holiday (a)',
        'bradford half holiday (b)',
        'airewharfe wednesday pairs',
        'airewharfe wednesday pairs (a)',
        'airewharfe wednesday pairs (b)',
        'leeds thursday vets',
        'leeds thursday vets (a)',
        'leeds thursday vets (b)',
        'leeds ladies',
        'leeds saturday',
        'leeds saturday (a)',
        'leeds saturday (b)',
        'bradford saturday',
        'bradford saturday (a)',
        'bradford saturday (b)',

        // Other leagues
        'bradford monday',
        'bradford monday (a)',
        'bradford monday (b)',
        'airewharfe vets',
        'mirfield',
        'mirfield (a)',
        'mirfield (b)',
        'mirfield (c)',
        'spen valley',
        'spen valley (a)',
        'spen valley (b)',
        'spen valley (c)',
        'spen valley (d)',
        'bradford vets',
        'bradford vets (a)',
        'bradford vets (b)',
    ],
    historicTeamInfo: [
        {
            teamNames: ['leeds monday combined', 'airewharfe monday'],
        },
        {
            teamNames: ['leeds tuesday vets'],
        },
        {
            teamNames: ['leeds tuesday'],
        },
        {
            teamNames: ['leeds half holiday', 'bradford half holiday'],
        },
        {
            teamNames: ['airewharfe wednesday pairs'],
        },
        {
            teamNames: ['leeds thursday vets'],
        },
        {
            teamNames: ['leeds saturday', 'bradford saturday'],
        },
    ],
};
